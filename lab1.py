...
# Инициализация
event_queue = deque()  # Очередь событий (DEQ)
current_time = 0.0
...
# Добавление события в очередь с приоритетом
def add_event(event_type, time):
    # Вставляем событие в правильное место в очереди
    for i, (event_time, _) in enumerate(event_queue):
        if time < event_time:
            event_queue.insert(i, (time, event_type))
            return
    event_queue.append((time, event_type))

# Инициализация первого события прихода клиента
next_arrival_time = generate_arrival_time()
add_event('arrival', next_arrival_time)

# Основной цикл моделирования
while current_time < simulation_time:
    # Получаем следующее событие
    event_time, event_type = event_queue.popleft()
    ...
    if event_type == 'arrival':
        ...
        if not server_busy:
            ...
            add_event('departure', current_time + service_time)
            server_busy = True
        else:
